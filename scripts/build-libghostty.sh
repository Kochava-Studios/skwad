...
# Build libghostty as universal (arm64 + x86_64) static library for Skwad.
...
build_arch() {
    ...
    # Note: sudo may be required due to Xcode 26 Metal toolchain permission bug
    (cd "${WORKDIR}/ghostty" && sudo zig build "${ZIG_FLAGS[@]}" -Dtarget="${arch}-macos" -p "${outdir}")
...
# Copy headers from source (the build doesn't install ghostty.h)
if [ -d "${WORKDIR}/ghostty/include" ]; then
    cp -r "${WORKDIR}/ghostty/include/"* "${VENDOR_DIR}/include/"
fi
